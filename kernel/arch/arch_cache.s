...
_dcache_flush_all_level_loop:
    ...
    lsr x2, x0, x2
    ands xzr, x2, #7
    beq _dcache_flush_all_done       // Stop if there are no caches at the current level
    ands xzr, x2, #6
    beq _dcache_flush_all_level_loop // Skip to next level if there is no data/unified cache at the current level
...
_dcache_flush_all_way_loop:
    mov x8, x5

_dcache_flush_all_set_loop:
    ...
    subs x8, x8, #1
    bge _dcache_flush_all_set_loop

    # Keep looping for all ways
    subs x4, x4, #1
    bge _dcache_flush_all_way_loop

    # Keep looping for all levels
    # ARMv8 supports a maximum of 7 levels
    add x1, x1, #1
    cmp x1, #7
    bne _dcache_flush_all_level_loop

    dsb sy
    isb sy

_dcache_flush_all_done:
    ldp fp, lr, [sp], #16
    add sp, sp, #16
    ret lr
