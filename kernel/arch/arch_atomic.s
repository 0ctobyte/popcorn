...
# x1 [in] - Bit to set
# x0 [out] - 0 on success, otherwise failure
.global arch_atomic_test_and_set_bit
.align 2
arch_atomic_test_and_set_bit:
    mov x2, x1
    mov x1, x0
    ldxr x0, [x1]
    tst x0, x2
...
.global arch_atomic_inc
.align 2
arch_atomic_inc:
    ldxr x1, [x0]
    add x2, x1, #1
    stxr w3, x2, [x0]
    cbnz w3, arch_atomic_inc
...
.global arch_atomic_dec
.align 2
arch_atomic_dec:
    ldxr x1, [x0]
    sub x2, x1, #1
    stxr w3, x2, [x0]
    cbnz w3, arch_atomic_dec
