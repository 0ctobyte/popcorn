...
#define list_entry(node, struct_type, struct_member) ((node) != NULL ? (structof(node, struct_type, struct_member)) :\
    NULL)
...
#define list_insert_after(list, prev, node)          (list_insert_here(list, prev, ((prev) == NULL) ? (list)->first :\
    (prev)->next, node))
#define list_insert_before(list, next, node)         (list_insert_here(list, ((next) == NULL) ? (list)->last :\
    (next)->prev, next, node))
...
// if the list entry and key match (in w/e definition of the word "match" is employed) and either LIST_COMPARE_GT or
// LIST_COMPARE_LT otherwise This routine will return a valid node if one is found otherwise it shall return NULL
