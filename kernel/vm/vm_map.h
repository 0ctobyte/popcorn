...
// Mappings may not actually have a physical address range associated with it; physical pages are linked to mappings on
// demand
typedef struct {
    ...
    rbtree_node_t rb_hnode;  // Red/black tree linkage of mappings based on the size of the virtual address space hole
                             // after the mapping
...
typedef struct {
    ...
    rbtree_t rb_mappings;    // All contiguous virtual memory regions associated with this virtual memory space rooted
                             // in a red/black tree
    rbtree_t rb_holes;       // A red/black tree of mappings ordered by the holes in the virtual address space afer
                             // the mapping
    vaddr_t start, end;      // The start and end virtual addresses of the entire possible virtual space definied by
                             // this map
...
// Enter a mapping of the given size into object starting at offset with the specified protection and at the given
// virtual address. This routine will check to make sure the given virtual address and size can fit in the map since
// mapping entries cannot overlap. Return an error code if failed
...
// Given the map, virtual address and fault (i.e. access) type, returns the object, offset and protection of the
// virtual address
