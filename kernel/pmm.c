...
paddr_t pmm_alloc(void) {
    ...
    unsigned long frame = bit_find_contiguous_zeros(top->bitmap, 1);
...
void pmm_free(paddr_t addr) {
    ...
    unsigned long frame_num = ATOP((addr-MEMBASEADDR));
    unsigned long elem_num = GET_PAGEMAP_ARRAY_INDEX(frame_num);
    unsigned long rel_frame_num = GET_REL_FRAME_NUM(frame_num, elem_num);
