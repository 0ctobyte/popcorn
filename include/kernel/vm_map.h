...
// the root of which is contained in the map. Mappings are linked to a virtual memory object which
...
int vm_map_enter_at(vm_map_t *vmap, vaddr_t vaddr, size_t size, vm_object_t *object, vm_offset_t offset, vm_prot_t prot);
...
// Return an error code if failed
int vm_map_enter(vm_map_t *vmap, vaddr_t *vaddr, size_t size, vm_object_t *object, vm_offset_t offset, vm_prot_t prot);

// Remove the given virtual address range from the map
int vm_map_remove(vm_map_t *vmap, vaddr_t start, vaddr_t end);

// Sets new protection attributes for the given virtual address range in the specified map
int vm_map_protect(vm_map_t *vmap, vaddr_t start, vaddr_t end, vm_prot_t new_prot);

// Given the map, virtual address and fault (i.e. access) type, returns the object, offset and protection of the virtual address
int vm_map_lookup(vm_map_t *vmap, vaddr_t vaddr, vm_prot_t fault_type, vm_object_t *object, vm_offset_t *offset, vm_prot_t *prot);
